...
エンドフィールドモードには、現行データで確認できた30名と、学習向けに選んだ各12件（合計360件）の英語・日本語テキストを収録しています。テキストの出典は[Warfarin Wiki](https://warfarin.wiki/en/operators)です。ゲーム音声は取得・転載せず、端末の一般的なブラウザ音声合成で英文を読み上げます。
...
非公式の学習・非商用プロトタイプです。ゲーム名、キャラクター、台詞、音声などの権利は各権利者に帰属します。エンドフィールドのゲームテキスト・キャラクターデータの権利はHYPERGRYPH / GRYPHLINEに帰属し、同モードの読み上げ音声は原作ゲーム音声ではありません。
